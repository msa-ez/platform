...
class Gitlab extends Git {
      ...
      let repoInfo = me
        ...
        .then(async function (info) {
          let file = await axios
            .get(
              `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                info.id
  ...
  getUserInfo() {
    ...
    return new Promise(async function (resolve, reject) {
      let getUserInfo = await axios
        .get(`https://gitlab.${me.getOrigin()}/api/v4/user`, {
          headers: me.getHeader(),
        })
        .then(function (res) {
          resolve(res.data);
  ...
  getUser(org) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let getUserInfo = await axios
        .get(
          `https://gitlab.${me.getOrigin()}/api/v4/users?username=${userName}`
        )
        .then(function (res) {
          resolve(res.data[0]);
        })
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let userInfo = await me
        ...
        .then(async function (info) {
          const result = axios
            .get(
              `https://gitlab.${me.getOrigin()}/api/v4/users/${
                info.id
              }/projects?search=${repo}`,
              { headers: me.getHeader() }
            )
            .then((res) => {
  ...
  createRepo(org, repo, userName) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      var createProjectRes = await axios
        .post(
          `https://gitlab.${me.getOrigin()}/api/v4/projects`,
          newProjectData,
          { headers: me.getHeader() }
  ...
  getRepo(org, repo) {
    let me = this;
    return new Promise(async function (resolve, reject) {
      let result = await me.getRepoId(repo, org).then((res) => {
        if (res.id == null) {
          reject({
            response: {
              status: 404,
  ...
  getFile(org, repo, filePath, repoId) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let id = null;
      if (org.includes("msa-ez")) id = "root";

      if (!repoId) {
        let repoInfo = await me.getRepoId(repo, id).then(function (info) {
          repoId = info.id;
        });
      }
      ...
      // .then(async function (info) {
      let url = `https://gitlab.${me.getOrigin()}/api/v4/projects/${repoId}/repository/files/${encodeURIComponent(
        filePath
      )}?ref=main`;
      let file = await axios
  ...
  getFiles(options) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let result = [];
      let repoInfo = me
        .getRepoId(options.repo, options.org)
        .then(async function (info) {
          let firstPage = await axios
            .get(
              `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                info.id
              }/repository/tree?ref=${options.name}&id=${
                info.id
              }&page=1&per_page=100&recursive=true`,
              { headers: header }
            )
            .then(async function (firstRes) {
              ...
              if (totalPages == 1) resolve(result);

              for (var i = 2; i <= totalPages; i++) {
                let otherPages = await axios
                  .get(
                    `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                      info.id
                    }/repository/tree?ref=${options.name}&id=${
                      info.id
                    }&page=${i}&per_page=100&recursive=true`,
                    { headers: header }
                  )
  ...
  setGitList(element, repository, gitRepoUrl) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      //     userId =
      // }
      let repoInfo = await me.getRepoId(repo, org).then(function (info) {
        repoId = info.id;
      });
      let callCnt = 0;
      ...
      filteredElement.forEach(async function (ele) {
        ...
        let result = await me
          .getFile(org, repo, ele.path, repoId)
          .then(async function (res) {
            if (isToppingSetting) {
              try {
                ...
                if (!gitTemplateContents[elePath])
                  gitTemplateContents[elePath] = null;
                ...
                if (filteredElement.length == callCnt) {
                  Object.keys(gitTemplateContents).forEach(function (fileName) {
                    ...
                    gitToppingList[gitRepoUrl][fileName].content =
                      gitTemplateContents[fileName];
                  });
              ...
              try {
                ...
                templateFrameWorkList[gitRepoUrl][ele.path].requestUrl =
                  res.url;

                // var gitSha = await axios.get(ele.url, { headers: me.githubHeaders });
                if (!gitTemplateContents[ele.path])
                  gitTemplateContents[ele.path] = null;
                gitTemplateContents[ele.path] = res.data;
              } catch (e) {
                ...
                callCnt++;
                if (filteredElement.length == callCnt) {
                  manifestsPerBaseTemplate[gitRepoUrl] =
                    manifestsPerTemplate[gitRepoUrl];
                  Object.keys(gitTemplateContents).forEach(function (fileName) {
                    if (!templateFrameWorkList[gitRepoUrl][fileName]) {
                      templateFrameWorkList[gitRepoUrl][fileName] = {};
                    }
                    templateFrameWorkList[gitRepoUrl][fileName].content =
                      gitTemplateContents[fileName];
                  });
                  console.log(
                    `>>> Generate Code] Template(${gitRepoUrl}) DONE`
                  );
                  let result = {
  ...
  setPushList(options) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      gitlabFilePathList.forEach(function (elData) {
        if (
          !options.generateCodeLists.find(
            (element) => element.fullPath == elData
          )
        ) {
          var action = {
            action: "delete",
  ...
  push(options) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let userInfo = me
        .getRepoId(options.repo)
        .then(async function (info) {
          var pushtoMain = await axios
            .post(
              `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                info.id
              }/repository/commits`,
            ...
            .then(async (res) => {
              let templateBranch = await me
                .getTemplateBranch(options.org, options.repo)
                .then(() => {
                  // res.data.tree = true
                  resolve(res);
                })
                .catch(async (error) => {
                  if (error.response.status === 404) {
                    let createBranch = await me
                      .createBranch(options.org, options.repo, "template")
                      .then(async (result) => {
  ...
  createRelease(obj) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      let userInfo = me
        .getRepoId(obj.repo)
        .then(async function (info) {
          let templateBranch = await axios
            .post(
              `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                info.id
              }/releases`,
        ...
        .then(async function (info) {
          let templateBranch = await axios
            .post(
              `https://gitlab.${me.getOrigin()}/api/v4/projects/${
                info.id
              }/repository/branches?branch=${branch}&ref=main`,
              {},
              { headers: me.getHeader() }
            )
            .then((res) => {
  ...
  getOrgList() {
    ...
    return new Promise(async function (resolve, reject) {
      const result = await axios
        .get(`https://gitlab.${me.getOrigin()}/api/v4/groups`, {
          headers: me.getHeader(),
        })
        .then(async (res) => {
          let result = {
