...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async oldGetClazzList() {
                var me = this
                me.showLoading = true
                this.$minioClient.listObjects(`labs-${me.getTenantId()}`, ``, false)
                    .on('data', function (rootObj) {
                        // console.log(rootObj)
                        if (rootObj.prefix) {
                            var courseList = rootObj.prefix.replace('/', '')
                            me.getCourseList.push(courseList)
                            var getMeta = function (classObj) {
                                if (classObj.name && classObj.name.includes('Class_Metadata') && classObj.size != 0) {
                                    // console.log(classObj.name)
                                    me.$minioClient.getObject(`labs-${me.getTenantId()}`, classObj.name, function (err, dataStream) {
                                        var chunks = null
                                        if (err) {
                                            return console.log(classObj.name)
                                        }
                                        ...
                                        dataStream.on('end', function () {
                                            var Uint8Array
                                            var string = new TextDecoder("utf-8").decode(chunks);
                                            var classMetadata = JSON.parse(string)
                                            me.setClassList(classMetadata, classObj)
                                        })
                            ...
                            me.$minioClient.listObjects(`labs-${me.getTenantId()}`, rootObj.prefix + 'classes/', false)
                                .on('data', function (courseObj) {

                                    me.$minioClient.listObjects(`labs-${me.getTenantId()}`, courseObj.prefix, false)
                                        .on('data', getMeta)
                                        .on('end', function () {
                            ...
                            //planned
                            me.$minioClient.listObjects(`labs-${me.getTenantId()}`, rootObj.prefix + 'classes/planned/', false)
                                .on('data', function (courseObj) {
                                    me.$minioClient.listObjects(`labs-${me.getTenantId()}`, courseObj.prefix, false)
                                        .on('data', getMeta)
                                        .on('end', function () {
                                            console.log("planned done")
                                        })
                                })
                                .on('error', function (err) {
                                    alert(err);
