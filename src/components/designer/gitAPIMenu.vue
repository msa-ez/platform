  ...
  <div :style="isSIgpt ? 'display: none;' : ''">
    ...
    >
      <v-layout style="top: -170px" align-center pr-4>
        <v-icon v-if="gitSnackBar.icon" class="pr-3" dark large>{{
          gitSnackBar.icon
        }}</v-icon>
        <v-layout column>
          ...
          <div v-if="gitSnackBar.multiLineText">
            {{ gitSnackBar.multiLineText }}
          </div>
        </v-layout>
      </v-layout>
      <template v-slot:action="{ attrs }">
        <v-btn
          v-if="
            gitSnackBar.title == 'Warning' &&
            gitSnackBar.Text == 'Nothing changed' &&
            !isOnPrem
          "
          color="white"
          outlined
        ...
        <v-btn
          ...
          v-bind="attrs"
          @click="
            (gitSnackBar.show = false), (gitSnackBar.multiLineText = null)
          "
        >
          Close
        </v-btn>
    ...
    <v-card
      v-if="gitMenuMode == 'push'"
      :disabled="!isListSettingDone"
      style="width: 666px"
    >
      ...
      <v-tabs :key="gitTabKey" v-model="gitTab" vertical>
        <v-tab
        ...
        >
          ...
          <v-tooltip right>
            <template v-slot:activator="{ on }">
              <v-icon
                v-on="on"
                v-if="item.tabKey == 'setAccount' && githubTokenError"
                end
                color="error"
            ...
            <v-card flat style="padding: 10px">
              <div>
                ...
                >
                  <v-row no-gutters v-if="gitRefRepoUrl">
                    ...
                    <v-col cols="9">
                      <v-text-field readonly dense v-model="gitRefRepoUrl">
                        <template
                          v-if="!isFirstCommit || isExistRepo"
                          v-slot:append-outer
                        >
                          <v-tooltip left>
                            <template v-slot:activator="{ on }">
                  ...
                  <v-radio
                    v-if="!isExistRepo"
                    :style="
                      gitRadios == 'fork' ? 'position:absolute; top:115px;' : ''
                    "
                ...
                <div
                  :key="copyInfoKey"
                  v-if="item.tabKey == 'info'"
                  style="margin: -10px"
                >
                  <div>
                    <template>
                      <v-tabs grow v-model="githubIdeTabs">
                        <v-tab> Cloud IDE </v-tab>
                        <v-tab> Local IDE </v-tab>
                      </v-tabs>
                    </template>
                        ...
                        <v-card flat :key="gitPodError">
                          ...
                          >
                            <template v-slot:append>
                              <v-icon
                                color="success"
                                v-if="isCopied == 'gitMerge_Command'"
                              >
                                mdi-check
                              </v-icon>
                              <v-icon
                                v-else
                                @click="copytoClipBoard('gitMerge_Command')"
                              >
                                mdi-content-copy
                              </v-icon>
                            </template>
                        ...
                        <v-card flat>
                          ...
                          >
                            <template v-slot:append>
                              <v-icon
                                color="success"
                                v-if="isCopied == 'gitClone_Command'"
                              >
                                mdi-check
                              </v-icon>
                              <v-icon
                                v-else
                                @click="copytoClipBoard('gitClone_Command')"
                              >
                                mdi-content-copy
                              </v-icon>
                            </template>
                ...
                >
                  ...
                  <div style="margin: 10px">
                    <div
                      ...
                      "
                    >
                      <v-btn small text color="primary" @click="logoutToGit()"
                        >sign out</v-btn
                      >
                    </div>
                    <div
                ...
                <span
                  v-if="
                    item.tabKey == 'push' || item.tabKey == 'openGithubEditor'
                  "
              ...
              <div style="float: right">
                <div
                  v-if="
                    (!isPushing &&
                      item.tabKey == 'setFirstRepo' &&
                      gitRadios == 'createNewRepo') ||
                    (item.tabKey == 'changeRepo' && isExistRepoMessage == null)
                  "
                >
                  <v-btn
                    :disabled="!gitOrgName || !gitRepoName || isExistRepo"
                    color="primary"
                    @click="startCommit()"
                    >Create</v-btn
                  >
                ...
                <div>
                  <v-btn
                    v-if="
                      (item.tabKey == 'changeRepo' && isExistRepoMessage) ||
                      (item.tabKey == 'setFirstRepo' && isExistRepo)
                    "
                  ...
                  <v-btn
                    ...
                    :disabled="
                      !isListSettingDone ||
                      !(gitOrgName && gitRepoName) ||
                      isExistRepo ||
                      (isPrivilegedUser && !information.gitOrgName)
                    "
                    text
                    color="primary"
                    @click="startFork()"
                  ...
                  <v-btn-toggle v-if="!isPushing && item.tabKey == 'push'">
                    ...
                    <v-btn
                      :disabled="
                        !isListSettingDone || !(gitOrgName && gitRepoName)
                      "
                      color="primary"
                      @click="startCommit()"
                      >{{ pushBtnName }}
                  ...
                  <span
                    v-if="
                      isPushing &&
                      (item.tabKey == 'push' || item.tabKey == 'setFirstRepo')
                    "
                  >
                    <b
                      :key="commitStep"
                      v-if="commitStepText != null && commitStep != null"
                      >{{ commitStepText }}({{ commitStep }}%) ...
                    </b>
                    <v-progress-circular
                      indeterminate
                      color="primary"
    ...
    <v-dialog v-model="showLoginCard"
      ><component
        :is="provider ? 'LoginByAcebase' : 'Login'"
        :onlyGitLogin="true"
        @login="showLoginCard = false"
    /></v-dialog>
  </div>
</template>
...
import LoginByAcebase from "../oauth/LoginByAcebase";
...
export default {
  ...
  data() {
    return {
      ...
      pushType: "Force Push",
      pushBtnitems: [{ title: "Push" }, { title: "Force Push" }],
      gitlabGroupIdList: {},
      gitlabDomain: null,
      gitHeaders: {},
  ...
  computed: {
    releaseTagPath() {
      return !this.editTemplateMode &&
        this.projectVersion &&
        !this.isFirstCommit
        ? `/releases/tag/${this.value.tag}`
        : "";
    },
    cloneTagPath() {
      return !this.editTemplateMode &&
    ...
    gitpodUrl() {
      ...
      if (this.isFirstCommit) {
        if (this.value) {
          if (
            this.value.forkedOrg &&
            this.value.forkedRepo &&
      ...
      if (this.isFirstCommit) {
        if (this.value) {
          if (!this.value.forkedOrg || !this.value.forkedRepo) {
            return this.git.getCloneCommand(
              this.gitOrgName,
              this.gitRepoName,
              this.cloneTagPath
            );
    ...
    gitTabItems() {
      ...
      if (!me.isGitLogin) {
        ...
        setTabItems = [
          { tab: "Repository", index: 0, tabKey: "changeRepo" },
          { tab: "Push", index: 1, tabKey: "push" },
          { tab: "Editor", index: 2, tabKey: "openGithubEditor" },
        ];
      } else {
        if (me.projectVersion) {
          setTabItems = [{ tab: "IDE", tabKey: "info", index: 0 }];
        } else if (me.isOwnModel) {
          if (me.isFirstCommit) {
            setTabItems = [
              { tab: "Repository", index: 0, tabKey: "setFirstRepo" },
              { tab: "Push", index: 1, tabKey: "push" },
              { tab: "IDE", tabKey: "info", index: 2 },
            ];
          } else {
            setTabItems = [
              { tab: "Repository", index: 0, tabKey: "changeRepo" },
          ...
          if (
            me.information.permissions[me.userInfo.uid].firstCommit == "false"
          ) {
            setTabItems = [
              { tab: "Repository", index: 0, tabKey: "changeRepo" },
              { tab: "Push", index: 1, tabKey: "push" },
  ...
  watch: {
    gitTab(newTabName) {
      var me = this;
      if (typeof newTabName == "string") {
        me.selectTab(newTabName);
      }
    },
    pushType(newVal) {
      if (newVal == "Push") {
    ...
    gitUserName: _.debounce(function () {
      if (
        (this.isOwnModel || this.isPrivilegedUser) &&
        (!this.firstSetUserName || !this.gitOrgName)
      ) {
  ...
  methods: {
    selectTab(tabName) {
      ...
      me.$app.try({
        context: me,
        async action(me) {
          const tabToSelect = me.gitTabItems.find(
    ...
    async getActionLogs() {
      ...
      me.$app.try({
        ...
        async action(me) {
          await me.git
            .getActionId(me.value.org, me.value.repo)
            .then(async (id) => {
              me.$EventBus.$emit(
                "setActionId",
                `https://github.com/${me.value.org}/${me.value.repo}/actions/runs/${id.run_id}/job/${id.job_id}`
              );
              await me.git
                .getActionLogs(me.value.org, me.value.repo, id)
                .then((result) => {
    ...
    async validateRepoBranch() {
      ...
      me.$app.try({
        context: me,
        async action(me) {
          const branches = await me.git
            .getBranch(me.value.org, me.value.repo, me.value.forkedTag)
            .then((result) => {
      ...
      me.$app.try({
        ...
        async action(me) {
          if (me.value.org && me.value.repo) {
            const tagList = await me.git
              .getTags(me.value.org, me.value.repo)
              .then((result) => {
                // console.log(tagList)
                result.data.forEach(async function (tag) {
                  if (me.value.forkedTag && tag.name == me.value.forkedTag) {
                    ...
                    let createBranchResult = await me.git.createBranch(
                      me.value.org,
                      me.value.repo,
                      templateBranchData
                    );
    ...
    async checkReleasedTags() {
      ...
      me.$app.try({
        ...
        async action(me) {
          if (me.projectVersion) {
            let result = await me.git
              .getReleasedTag(me.value.org, me.value.repo, me.value.tag)
              .then(() => {
    ...
    async startCommitTemplate() {
      ...
      me.$app.try({
        ...
        async action(me) {
          ...
          let gitRepoMainInfo = await me.git
            .getMainRepo(me.gitOrgName, me.gitRepoName)
            .then((result) => {
              setTimeout(function () {
            ...
            .catch(async (error) => {
              if (error.response) {
                if (
                  ...
                  //     auto_init: true
                  // }
                  let addNewRepo = await me.git
                    .createRepo(me.gitOrgName, me.gitRepoName, me.gitUserName)
                    .then(() => {
                      setTimeout(function () {
                        me.startCommitTemplateList(me.gitRepoName);
                      }, 3000);
              ...
              if (error.response) {
                if (
                  ...
                  if (forkedRepoName) {
                    ...
                    me.gitSnackBar.Text =
                      "A forked repository already exists. The name of the configured repository is changed to the name of the forked repository. If you want to push, proceed to push again";
    ...
    async startCommitWithGitLab() {
      ...
      try {
        ...
        if (me.projectId) {
          if (me.isServerModel) {
            if (!me.isPrivilegedUser) {
              ...
              me.setString(
                `db://definitions/${me.projectId}/information/permissions/${me.userInfo.uid}/gitOrgName`,
                me.gitOrgName
              );
              me.setString(
                `db://definitions/${me.projectId}/information/permissions/${me.userInfo.uid}/gitRepoName`,
                me.gitRepoName
              );
            }
          }
        ...
        var response1 = await axios
          .get(
            `https://gitlab.${me.gitlabDomain}/api/v4/users?username=${me.gitUserName}`,
            { headers: { Authorization: "Bearer " + me.gitToken } }
          )
        ...
        if (response1.data.length > 0) {
          ...
          var response2 = await axios
            .get(
              `https://gitlab.${me.gitlabDomain}/api/v4/users/${response1.data[0].id}/projects`,
              { headers: { Authorization: "Bearer " + me.gitToken } }
            )
            .catch(function (error) {
              me.commonError(error);
          ...
          if (!me.gitLabProjectId) {
            ...
            var createProjectRes = await me.$http.post(
              `https://gitlab.${me.gitlabDomain}/api/v4/projects`,
              newProjectData,
              { headers: { Authorization: "Bearer " + me.gitToken } }
            );
          ...
          if (me.gitLabProjectId) {
            ...
            var getFilePathList = await axios
              .get(
                `https://gitlab.${me.gitlabDomain}/api/v4/projects/${me.gitLabProjectId}/repository/tree?ref=${branchName}&id=${me.gitLabProjectId}&page=1&per_page=100&pagination=keyset&recursive=true`,
                { headers: { Authorization: "Bearer " + me.gitToken } }
              )
              .catch(function (error) {
                me.commonError(error);
            ...
            for (var i = 2; i <= totalPage; i++) {
              var getMoreFilePathList = await axios
                .get(
                  `https://gitlab.${me.gitlabDomain}/api/v4/projects/${me.gitLabProjectId}/repository/tree?ref=${branchName}&id=${me.gitLabProjectId}&page=${i}&per_page=100&pagination=keyset&recursive=true`,
                  { headers: { Authorization: "Bearer " + me.gitToken } }
                )
                .catch(function (error) {
                  me.commonError(error);
                });
              me.gitlabFileList = me.gitlabFileList.concat(
                getMoreFilePathList.data
              );
          ...
          me.showErrSnackBar(
            `userName Error, "${me.gitUserName}" does not exist`
          );
        }
      } catch (error) {
        console.error(error);
    ...
    async setPushListWithGitLab() {
      ...
      me.gitlabFilePathList.forEach(function (elData) {
        if (
          !me.generateCodeLists.find((element) => element.fullPath == elData)
        ) {
          var action = {
            action: "delete",
            file_path: elData,
      ...
      me.generateCodeLists.forEach(function (elData) {
        if (
          !me.gitlabFilePathList.find((element) => element == elData.fullPath)
        ) {
          var code;
          if (elData.code) {
            code = elData.code;
          ...
          ((me.copyChangedPathLists &&
            me.copyChangedPathLists.find(
              (element) => element.replace("for-model/", "") == elData.fullPath
            )) ||
            me.pushType != "Push")
        ) {
          var code;
      ...
      me.gitLabCommitAction.reduce(function (acc, current) {
        if (
          acc.findIndex(({ file_path }) => file_path === current.file_path) ===
          -1
        ) {
          acc.push(current);
        }
      ...
      if (setCommitActionList) {
        ...
        if (me.isNewProject) {
          ...
          var pushtoMainRes = await axios
            .post(
              `https://gitlab.${me.gitlabDomain}/api/v4/projects/${me.gitLabProjectId}/repository/commits`,
              committoMainData,
              { headers: me.gitHeaders }
            )
            .catch(function (error) {
              me.commonError(error);
            });
          ...
          if (pushtoMainRes) {
            var createBranchRes = await axios
              .post(
                `https://gitlab.${me.gitlabDomain}/api/v4/projects/${me.gitLabProjectId}/repository/branches?branch=template&ref=main`,
                {},
                { headers: me.gitHeaders }
              )
              .catch(function (error) {
            ...
            .catch(function (error) {
              if (
                ...
                if (
                  ...
                  errText =
                    errText + ", " + error.response.data.errors[0].message;
                }
                me.showErrSnackBar(errText);
              } else {
                me.showErrSnackBar(error.message);
        ...
        if (me.isServerModel) {
          if (!me.isPrivilegedUser) {
            me.setString(
              `db://definitions/${me.projectId}/information/firstCommit`,
              "false"
            );
          } else {
            me.setString(
              `db://definitions/${me.projectId}/information/permissions/${me.userInfo.uid}/firstCommit`,
              "false"
            );
          }
        }
    ...
    async startForkTemplate() {
      ...
      me.$app.try({
        ...
        async action(me) {
          ...
          let forkRes = await axios
            .post(
              `https://api.github.com/repos/${me.templateGitPath[3]}/${me.templateGitPath[4]}/forks`,
              forkData,
              { headers: me.gitHeaders }
            )
    ...
    async startForkWithGitHub() {
      ...
      me.$app.try({
        ...
        async action(me) {
          ...
          let forkRes = await axios
            .post(
              `https://api.github.com/repos/${me.gitForkOrgName}/${me.gitForkRepoName}/forks`,
              forkData,
              { headers: me.gitHeaders }
            )
            .catch(async function (error) {
          ...
          if (forkRes) {
            ...
            if (me.isExistRepo) {
              ...
              me.gitSnackBar.timeout = -1;
              me.gitSnackBar.Text =
                "Forked repository already exists on currently org.";
              me.gitSnackBar.Color = "warning";
              me.gitSnackBar.icon = "warning";
              me.gitSnackBar.title = "Warning";
    ...
    async startForkWithGitLab() {
      ...
      try {
        ...
        let forkRes = await axios
          .post(
            `https://gitlab.${me.gitlabDomain}/api/v4/projects/13/fork`,
            forkData,
            { headers: me.gitHeaders }
          )
          .catch(function (error) {
        ...
        async action(me) {
          if (me.isOnPrem) {
            ...
            if (
              ...
              localStorage.getItem("loginType") == "gitlab"
            ) {
              me.isGitLogin = true;
            }
          } else {
            ...
            if (
              localStorage.getItem("loginType") &&
              localStorage.getItem("loginType") == "github"
            ) {
              me.isGitLogin = true;
            }
          }
          ...
          if (localStorage.getItem("forkedModelInfo")) {
            if (
              localStorage.getItem("forkedModelInfo").includes("gitOrgName")
            ) {
              var beforeModelInfo = JSON.parse(
                JSON.parse(localStorage.getItem("forkedModelInfo"))
              );
          ...
          me.copyChangedPathLists = JSON.parse(
            JSON.stringify(me.changedPathListsForGit)
          );
          ...
          if (me.value && !me.isPrivilegedUser) {
            ...
            if (!me.information.useIdeItem) {
              if (me.isServerModel) {
                me.setString(
                  `db://definitions/${me.projectId}/information/useIdeItem`,
                  "gitpod"
                );
              }
            ...
            if (
              ...
              me.gitOrgName =
                me.information.permissions[me.userInfo.uid].gitOrgName;
              me.gitOrganizations.push(me.gitOrgName);
              if (me.information.permissions[me.userInfo.uid].gitRepoName) {
                me.gitRepoName =
                  me.information.permissions[me.userInfo.uid].gitRepoName;
              }
            } else {
              me.gitOrganizations = [];
              me.gitOrgName = me.gitUserName;
          ...
          if (
            ((me.isOwnModel || me.isPrivilegedUser) && !me.isFirstCommit) ||
            me.editTemplateMode
          ) {
            me.gitTab = "Push";
          } else {
          ...
          if (me.ShowCreateRepoTab) {
            ...
            me.gitSnackBar.multiLineText =
              "Click the Create button to create the github repository and push your code.";
    ...
    openGuide() {
      window.open(
        `https://github.com/TheOpenCloudEngine/msaschool/wiki/Github-Personal-Access-Token-%EB%B0%9C%ED%96%89-%EB%B0%A9%EB%B2%95`,
        "_blank"
      );
    },
    async checkRepoExist() {
      ...
      me.$app.try({
        context: me,
        async action(me) {
          let gitRepo = await me.git
            .getRepo(me.gitOrgName, me.gitRepoName)
            .then((result) => {
              me.isExistRepo = true;
              me.isExistRepoMessage = "The repository name already exist";
            })
    ...
    isGitLoginComplete() {
      ...
      me.isGitLogin = true;
      // me.gitMenuMode = "push"
      if (!me.isGitLogin) me.gitTab = "Push";
    },
      ...
      if (me.isLogin) {
        ...
        if (type == "gitpod") {
          if (me.showOriginMode) {
            targetUrl = me.isVersionBranch
              ? me.releaseTagPath == ""
                ? `${me.gitpodUrlOrigin}/tree/branch-${me.value.forkedTag}`
                : `${me.gitpodUrlOrigin.replace(
                    me.releaseTagPath,
                    ""
                  )}/tree/branch-${me.value.forkedTag}`
              : me.gitpodUrlOrigin;
            window.open(targetUrl, "_blank");
          } else {
            targetUrl = me.isVersionBranch
              ? me.releaseTagPath == ""
                ? `${me.gitpodUrl}/tree/branch-${me.value.forkedTag}`
                : `${me.gitpodUrl.replace(me.releaseTagPath, "")}/tree/branch-${
                    me.value.forkedTag
                  }`
              : me.gitpodUrl;
    ...
    async startCommit() {
      ...
      if (me.editTemplateMode) {
        ...
        // 초기 커밋시 모델 저장 기능
        // if(!me.isServerModel){
        //     let canvas = getParent(this.$parent, "event-storming-model-canvas");
        //     canvas.saveComposition('save')
        // } else {
    ...
    async startCommitWithGit(branch) {
      ...
      me.$app.try({
        ...
        async action(me) {
          ...
          me.commitStepText = "Check Git Repo";
          let gitRepoInfo = await me.git
            .getTemplateBranch(me.gitOrgName, me.gitRepoName, branch)
            .then(async (result) => {
              if (!me.isFirstCommit && result) {
                ...
                gitTreeList = await me.git
                  .getFiles(treeOptions)
                        ...
                        if (!list) {
                          ...
                          let commitResult = await me.git
                            ...
                            .then(async (commit) => {
                              ...
                              let commitSha =
                                commit.data.sha || commit.data.commit.id;
                              me.$EventBus.$emit("getCommitId", commitSha);
                              let pushResult = await me.git
                                .push(options)
                                .then(function () {
                                  me.commitStepText = null;
                                  me.isPushing = false;
      ...
      me.$app.try({
        ...
        async action(me) {
          ...
          Object.keys(me.editTemplateList).forEach(async function (tmpPath) {
            ...
            pushCnt++;
            let res = me.git
              .pushFile(me.gitOrgName, me.gitRepoName, data)
              .then(async (result) => {
                let pushData = {
                  path: tmpPath,
                  mode: "100644",
              ...
              let pushResult = await me.git
                ...
                .then(function (push) {
                  ...
                  if (me.autoApplyTemplate) {
                    var platformPath = me.git.getUrl(
                      me.gitOrgName,
                      me.gitRepoName
                    );
