...
\`\`\`json
{
    ...
    // 기존의 Aggregate나 ValueObject와 유사한 로그인 경우, 해당 이름을 그대로 활용할 수 있습니다.
    ...
    "aggregateIdToIncludeAsValueObejct": "<aggregateId>"|null,
...
\`\`\`json
{"objectName":"PointUsing","aggregateIdToIncludeAsValueObejct":"Customer","debeziumLogCommandName":"CreatePointUsing","debeziumLogEventName":"PointUsingCreated","eventsToTriggerDebeziumLogCommand":[],"commandsToTriggerByDebeziumLogEvent":[{"eventId":"cmd-update-customer","relatedAttribute":"point_balance","reason":"To update customers' point_balance information"}]}
\`\`\`
...
`
    ...
    createModel(text){
        ...
        const getCommandGuidesToUse = (commandGuides) => {
            const objectNameToString = (objectName, aggregateIdToIncludeAsValueObejct) => {
                if(aggregateIdToIncludeAsValueObejct && aggregateIdToIncludeAsValueObejct !== "null" && aggregateIdToIncludeAsValueObejct.length > 0)
                    return `'${aggregateIdToIncludeAsValueObejct}' Aggregate 내부에 ValueObject로 '${objectName}' 이름으로 생성해주세요.`
                else
                    return `다음 Aggregate 내부에 작성해주세요. 해당 Aggregate가 없을 경우, 생성해주세요.: ${objectName}`
            }
            ...
            return `${objectNameToString(commandGuides.objectName, commandGuides.aggregateIdToIncludeAsValueObejct)}
