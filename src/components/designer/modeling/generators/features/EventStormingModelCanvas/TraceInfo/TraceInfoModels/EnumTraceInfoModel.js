...
export default class EntityTraceInfoModel extends TraceInfoModelAbstract {
    ...
    getOriginalRefs() {
        if(!this.__isValidEnumTraceValue(this.value)) {
            return null;
        }
        ...
        // TYPE 1: 추적성 정보에 Enum 참조 정보가 이미 존재함(프로젝트 초안을 기반으로 생성됨)
        if (this.__isValidEnumTraceInfo(traceInfo, boundedContextTraceName, this.value.traceName)) {
            return traceInfo.structureRefs[boundedContextTraceName].enumerations[this.value.traceName];
        }
        
        // TYPE 2: 추적성 정보에 Enum 참조 정보가 없으며, 인덱스 맵핑 정보가 존재함(예외적인 상황)
        ...
        // TYPE 3: 추적성 정보에 Enum 참조 정보가 없으며, 인덱스 맵핑 정보도 없음(A2A를 기반으로 생성)
        if(this.value.refs) {
    ...
    __isValidEnumTraceValue(value) {
        return value && value.id && value.traceName;
    }
    __isValidEnumTraceInfo(traceInfo, boundedContextName, enumTraceName) {
