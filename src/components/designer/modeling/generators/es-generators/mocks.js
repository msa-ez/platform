 ...
 * // 가능한 이벤트 스토밍 서비스 종류: librarySevice
 ...
 * console.log(getEsValue("librarySevice", ['command', 'event', 'view']))
 * 
 ...
 */
export const getEsValue = (targetService="librarySevice", typesToExcludeFilter=[]) => {
    ...
    const targetEsValue = JSON.parse(JSON.stringify(esValues[targetService]))
    const filteredElements = Object.values(targetEsValue.elements).filter(element => {
        return !typesToExcludeFilter.some(type => element._type.toLowerCase().includes(type.toLowerCase()));
    });
