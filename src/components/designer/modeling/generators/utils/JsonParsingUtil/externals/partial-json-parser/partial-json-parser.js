...
// * 외부 모듈 수정 내역
// - '{"items": ["apple", "banana", "or'와 같이 전달시에 {"items": ["apple"]}과 같이 가장 앞의 요소만 파싱되던 성능 이슈 해결
  ...
  strip = function strip(tokens) {
    ...
    switch (lastToken.type) {
      ...
      case 'string':
        ...
        if (tokenBeforeTheLastToken && tokenBeforeTheLastToken.type === 'delimiter') {
          ...
          tokens = tokens.slice(0, tokens.length - 1);
          return strip(tokens);
        } else if (tokenBeforeTheLastToken && tokenBeforeTheLastToken.type === 'brace' && tokenBeforeTheLastToken.value === '{') {
          tokens = tokens.slice(0, tokens.length - 1);
          return strip(tokens);
        }
        break;
