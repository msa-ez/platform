            ...
            filteredCustomToppingLists(){
                try{
                    ...
                    list.forEach(function(str){
                        let match = str.split("/")[str.split("/").length-1]
                        // const match = str.match(/^http[s]?\:\/\/(github.com)\/([-_A-Za-z0-9\s]+)\//i);
                        obj[match] = list.filter(x=>x.includes(match));
                    });
                    return obj
                }catch(e){
    ...
    - name: Test
      run: |
        mvn test -f ./${me.openCode[0].name}`
            ...
            applyTemplateInMarket(temp, toppingInfo){
                this.changePlatformToForkedRepo(`${temp.templatePath}`, true, null, toppingInfo)
