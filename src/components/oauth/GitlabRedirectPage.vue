...
export default {
  ...
  mounted: async function () {
    ...
    let origin = window.GIT_URL
      ? window.GIT_URL
      : window.location.hostname.replace("www.", "");
    // console.log(parser.getAll())
    var getToken = await this.$http.post(
      `https://gitlab.${origin}/oauth/token`,
      {
    ...
    var getUsers = await this.$http.get(
      `https://gitlab.${origin}/api/v4/user?access_token=${accessToken}`
    );
  ...
  methods: {
    async postMessage(getUsers) {
      return new Promise(function (resolve) {
        ...
        window.localStorage.setItem("gitUserName", getUsers.data.username);
        window.localStorage.setItem("gitEmail", getUsers.data.email);
        window.localStorage.setItem("gitToken", getUsers.data.accessToken);
        window.opener.postMessage(
          {
            message: "gitlab-login",
            data: JSON.parse(JSON.stringify(getUsers)),
          },
          "*"
        );
        resolve();
