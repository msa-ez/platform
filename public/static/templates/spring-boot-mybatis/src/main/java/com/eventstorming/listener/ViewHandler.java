...
path: {{boundedContext.name}}/{{{options.packagePath}}}/listener
---
...
public class {{namePascalCase}}ViewHandler {
    ...
    {{#createRules}}
    @Override
    @KafkaListener(topics = "{{options.package}}", groupId = "{{../../name}}", containerFactory = "kafkaListenerContainerFactory")
    public void when{{when.namePascalCase}}_then_{{operation}}_{{_index}} (ConsumerRecord data, Acknowledgment acknowledgment) {
        try {
