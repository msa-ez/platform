...
import UMLClassModelCanvas from '@/components/designer/class-modeling/UMLClassModelCanvas.vue'
...
describe('UMLClassModelCanvas', () => {
  it('additively add the field descriptor from chat-gpt generated model', () => {
    const ui = shallowMount(UMLClassModelCanvas, {
        propsData: {
    ...
    debugger
    ui.vm.createModel(umlModel);

    let converted = ui.vm.value
    


    let aggregate = Object.values(converted.elements).find(ele => ele.name === "Customer")
    let additiveField = aggregate.fieldDescriptors.find(fd => fd.name == "phoneNumber")
